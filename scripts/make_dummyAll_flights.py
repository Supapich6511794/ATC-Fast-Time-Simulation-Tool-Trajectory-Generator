"""Generate an *all-flights* dummy file in the website's own export format.

Same export format as scripts/make_dummy_flights.py, but with two changes the
brief asks for:

  * **Every** filed AIP route becomes a flight (no 100-flight cap).
  * **1 FPL = 1 route** — each flight carries exactly one route (no
    multi-route grouping per city pair).
...
exactly what the AIP prescribes and resolve when the web app calls
/api/generate.

Outputs (round-trip through web/lib/flightFile.ts):
  * dummy_data/dummyAll_flights.csv      — stacked "FLIGHT n of N" ATC
    trajectory blocks (importer splits on ^ROUTE:).
  * dummy_data/dummyAll_flights.geojson  — one FeatureCollection; per flight
...
Run:  python scripts/make_dummyAll_flights.py
"""
...
def main() -> None:
    ...
    # 1 FPL = 1 route = 1 flight (no multi-route grouping).
    flights = []
    for i, (r, coords) in enumerate(usable):
        ...
        # Concrete departure/arrival runway from the chosen SID/STAR (expand a
        # "both parallels" STAR runway RWxxB to a real side), and anchor the
        # cosmetic climb/descent to that runway's threshold elevation.
        ...
        flights.append({
            ...
            "routes": [{
                "flight_key": f"{callsign}_{stamp}_R1",
                "route_str": r["route"],
                "rfl": rfl,
                "route_pts": route_pts,
    ...
    csv_path = OUT_DIR / "dummyAll_flights.csv"
    csv_path.write_text("\n\n\n".join(blocks) + "\n", encoding="utf-8")
    ...
    geojson_path = OUT_DIR / "dummyAll_flights.geojson"
    ...
    print(f"Flights: {len(flights)} | routes: {total} (1 FPL = 1 route) | "
          f"with SID/STAR: {proc}")
